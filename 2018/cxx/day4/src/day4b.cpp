...
int main(int argc, char *argv[]) {
  ...
  auto sleepyhead_it = std::max_element(
      guard_entries.begin(), guard_entries.end(),
      [](const auto &e1, const auto &e2) {
        auto hardest_min1 = find_sleepiest_minute(e1);
        auto hardest_min2 = find_sleepiest_minute(e2);
        return e1.sleeps[hardest_min1] < e2.sleeps[hardest_min2];
      });
