...
std::vector<claim::num_t> fabric[fabric_dimension][fabric_dimension];
std::map<claim::num_t, bool> overlaps;
...
int main(int argc, char *argv[]) {
  ...
  for (const auto &c : claims) {
    for (auto i = 0u; i < c.width; i++) {
      for (auto j = 0u; j < c.height; j++) {
        fabric[c.x + i][c.y + j].push_back(c.id);
      }
    }
    overlaps[c.id] = false;
  }
  ...
  for (auto i = 0u; i < fabric_dimension; i++) {
    for (auto j = 0u; j < fabric_dimension; j++) {
      for (const auto id : fabric[i][j]) {
        if (!overlaps[id] && fabric[i][j].size() > 1) {
          overlaps[id] = true;
        }
      }
    }
